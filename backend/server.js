...
app.get('/api/hr-stats', async (req, res) => {
  ...
  SELECT 
    ...
    SUM(CASE WHEN jobs.status = 'profiles-sent' THEN 1 ELSE 0 END) AS sent_profiles,
    SUM(CASE WHEN jobs.status = 'drive-done' THEN 1 ELSE 0 END) AS drives_done,
...
app.get('/api/jobs_sdudents-stats', async (req, res) => {
  ...
  try {
    ...
    SELECT 
        ...
        COUNT(DISTINCT applied_students.candidateID) AS total_applications_received,
        COUNT(DISTINCT CASE WHEN applied_students.status = 'eligible' THEN applied_students.candidateID END) AS profiles_sent,
        COUNT(DISTINCT CASE WHEN applied_students.status = 'not-attended' THEN applied_students.candidateID END) AS not_attended,
        COUNT(DISTINCT CASE WHEN applied_students.status = 'eligible' THEN applied_students.candidateID END) 
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'not-attended' THEN applied_students.candidateID END) AS attended,
        COUNT(DISTINCT CASE WHEN applied_students.status = 'placed' THEN applied_students.candidateID END) AS selected,
        (COUNT(DISTINCT CASE WHEN applied_students.status = 'eligible' THEN applied_students.candidateID END) 
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'not-attended' THEN applied_students.candidateID END))
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'placed' THEN applied_students.candidateID END) AS rejected,
        (COUNT(DISTINCT CASE WHEN applied_students.status = 'eligible' THEN applied_students.candidateID END) 
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'not-attended' THEN applied_students.candidateID END))
          - ((COUNT(DISTINCT CASE WHEN applied_students.status = 'eligible' THEN applied_students.candidateID END) 
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'not-attended' THEN applied_students.candidateID END))
          - COUNT(DISTINCT CASE WHEN applied_students.status = 'placed' THEN applied_students.candidateID END)
          + COUNT(DISTINCT CASE WHEN applied_students.status = 'placed' THEN applied_students.candidateID END)) AS pending
    FROM 
        hr_data
    LEFT JOIN jobs ON jobs.postedBy = hr_data.HRid
    LEFT JOIN applied_students ON applied_students.jobID = jobs.jobId
